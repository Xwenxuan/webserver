...
public:
    ...
    void (*cb_fun)(client_data*);/*定时器回调函数*/
...
public:
    ...
    tw_timer * add_timer(int timeout) {
        ...
        int ticks = 0;
        //下面根据待插入定时器的超时值计算它将在时间轮转动多少个滴答后触发
        
        ，并将该滴答数存储于变量ticks中。如果待插入定时器的超时值小于时间轮的槽间隔SI，则将ticks向上折合为1，否则为timeout/SI
        if(timeout < SI) {
            ticks = 1;
        }else {
            ticks = timeout / SI;
        }
        /*计算待插入的定时器在时间轮转动多少圈后被出发*/
        int rotation = ticks / N:
        /*计算待插入的定时器应该被插入哪个槽中*/
        int ts = (cur_slot + (ticks % N)) % n;
        /*创建新的定时器，它在事件轮动转动rotation圈之后出发，且位于第ts个槽上*/
        tw_timer * timer = new tw_timer(rotation,ts);
        /*如果ts个槽中无任何定时器，则把新建的定时器插入其中，并将该定时器设为头节点*/
        if(!slots[ts]) {
            printf("add timer,rotation is %d,ts is %d,cur_slot is %dd\n",rotation,ts,cur_slot);
            slots[ts] = timer;
        }else {
            //否则就将定时器插入第ts个槽中
            //这是头插法啊
